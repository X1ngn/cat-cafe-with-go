# 猫猫咖啡屋使用指南

## 🎯 快速开始

### 1. 安装依赖

```bash
# 安装 Redis
brew install redis

# 下载 Go 依赖
go mod download
```

### 2. 编译项目

```bash
make build
```

### 3. 启动 Redis

```bash
redis-server --daemonize yes
```

### 4. 启动 Agent Workers

在不同终端启动各个 Agent:

```bash
# 终端 1
./bin/cat-cafe --mode agent --agent 花花

# 终端 2
./bin/cat-cafe --mode agent --agent 薇薇

# 终端 3
./bin/cat-cafe --mode agent --agent 小乔
```

### 5. 使用交互式界面（推荐）

```bash
./bin/cat-cafe --mode ui
```

在交互界面中:

```bash
# 发送任务
> @花花 实现一个 HTTP 服务器
✓ 任务已发送给 花花
  任务ID: xxx

> @薇薇 审查代码安全性
✓ 任务已发送给 薇薇

> @小乔 设计登录页面
✓ 任务已发送给 小乔

# 查看可用 Agent
> /list

# 查看帮助
> /help

# 退出
> /exit
```

---

## 📋 使用方式

### 方式一：交互式 UI（推荐）

最简单直观的使用方式，支持实时交互。

**启动 UI:**
```bash
./bin/cat-cafe --mode ui
```

**发送任务:**
```bash
> @Agent名称 任务内容
```

**可用命令:**
- `/list` - 列出所有 Agent
- `/help` - 显示帮助
- `/exit` - 退出程序

### 方式二：命令行模式

适合脚本化和自动化场景。

**列出 Agent:**
```bash
./bin/cat-cafe --list
```

**发送任务:**
```bash
./bin/cat-cafe --send --to 花花 --task "实现HTTP服务器"
```

### 方式三：API 模式（推荐用于前端集成）

提供 RESTful API 接口，支持 Web 前端和其他客户端接入。

**启动 API 服务器:**
```bash
./bin/cat-cafe --mode api --port 8080
```

或使用便捷脚本（会自动启动所有服务）：
```bash
./start.sh
```

**API 特性:**
- ✅ 会话管理：创建、查询、删除会话
- ✅ 消息管理：发送消息、获取历史消息
- ✅ Agent 管理：查询可用的猫猫 Agent
- ✅ 实时回复：Agent 处理完成后自动将回复添加到会话
- ✅ 调用历史：追踪每个会话中调用了哪些 Agent，包含完整的 Prompt 和 Response
- ✅ 自动轮询：前端自动获取新消息，无需手动刷新

**调用历史功能:**

调用历史记录了会话中所有被调用的猫猫，包括：
- 用户直接 @ 的猫猫
- 猫猫互相调用的猫猫
- 每次调用的完整 Prompt（提示词）
- 每次调用的完整 Response（回复）

前端展示特性：
- 🔄 实时刷新：每 3 秒自动更新调用历史
- 📜 可滚动列表：支持查看所有调用记录
- 📋 可展开详情：点击记录查看完整的 Prompt 和 Response
- 🎯 智能去重：每只猫猫只显示一次（在摘要视图中）

**前端实时消息推送:**

前端使用智能轮询机制自动获取新消息：
- 发送消息后：每 1 秒快速轮询，快速获取猫猫回复
- 收到回复后：切换到每 3 秒慢速轮询，节省资源
- 自动滚动：新消息到达时自动滚动到底部
- 无需刷新：消息自动显示，提供流畅的聊天体验

**基本使用流程:**

1. **创建会话**
```bash
curl -X POST http://localhost:8080/api/sessions
```

2. **发送消息并提及猫猫**
```bash
curl -X POST http://localhost:8080/api/sessions/sess_xxx/messages \
  -H "Content-Type: application/json" \
  -d '{
    "content": "@花花 你好，请帮我设计一个系统",
    "mentionedCats": ["cat_001"]
  }'
```

3. **获取消息列表（包含 Agent 回复）**
```bash
curl http://localhost:8080/api/sessions/sess_xxx/messages
```

**消息类型:**
- `user`: 用户发送的消息
- `cat`: Agent（猫猫）的回复
- `system`: 系统消息（如"花花已加入对话"）

**前端功能:**
- 🎨 头像显示：用户和猫猫都有专属头像
- 💬 @ 提及：输入 @ 弹出猫猫选择菜单
- 🔄 实时更新：消息自动刷新，无需手动操作
- 📊 调用历史：实时显示所有被调用的猫猫，支持展开查看详情
- 📝 Markdown 渲染：支持粗体、斜体、代码块、列表、表格等格式
- 📱 响应式设计：适配不同屏幕尺寸

**详细 API 文档:** 参见 `frontend/docs/API.md`

---

## 🔄 Agent 协作机制

### Agent 如何相互调用

Agent 在完成任务后，可以在输出中使用 **@标记** 来调用其他 Agent，实现自动化工作流。

### @标记语法

```
@Agent名称 任务内容
```

**规则:**
- @标记必须在行首
- 每行一个 @标记
- 格式: `@猫猫名 任务内容`

### 特殊标记

**@铲屎官** - 返回给用户，表示所有工作完成:
```
@铲屎官 任务完成，请查看结果
```

### 协作示例

#### 示例 1: 花花调用薇薇审查

花花输出:
```
【系统设计】
用户认证系统架构设计完成...

@薇薇 请审查这个系统的安全性
[架构文档内容]
```

薇薇会自动收到任务并开始审查。

#### 示例 2: 薇薇发现问题，要求花花修复

薇薇输出:
```
【审查报告】
发现3个安全问题:
1. 密码未加密
2. 缺少 CSRF 防护
3. Session 管理不当

@花花 请修复这些安全问题
[详细问题说明]
```

花花会自动收到任务并修复问题。

#### 示例 3: 完整工作流

```
用户: @花花 开发用户登录系统

花花 → 设计架构
    ↓
    @薇薇 审查安全性

薇薇 → 发现问题
    ↓
    @花花 修复问题

花花 → 修复完成
    ↓
    @小乔 设计界面

小乔 → 设计完成
    ↓
    @铲屎官 项目完成
```

### Agent 提示词配置

每个 Agent 的系统提示词中都包含了协作机制说明，告诉它们如何使用 @标记。

查看提示词文件:
- `prompts/calico_cat.md` - 花花
- `prompts/lihua_cat.md` - 薇薇
- `prompts/silver_cat.md` - 小乔

---

## ⚙️ 配置

编辑 `config.yaml`:

```yaml
agents:
  - name: "花花"
    pipe: "pipe_huahua"
    exec_cmd: "./minimal-claude"
    system_prompt_path: "prompts/calico_cat.md"

  - name: "薇薇"
    pipe: "pipe_weiwei"
    exec_cmd: "./minimal-codex"
    system_prompt_path: "prompts/lihua_cat.md"

  - name: "小乔"
    pipe: "pipe_xiaoqiao"
    exec_cmd: "./minimal-gemini"
    system_prompt_path: "prompts/silver_cat.md"

redis:
  addr: "localhost:6379"
  password: ""
  db: 0
```

---

## 📖 使用方法

### 列出所有 Agent

```bash
./bin/cat-cafe --list
```

输出:
```
🐱 可用的 Agent:

  花花 (claude)
    管道: pipe_huahua
    执行命令: ./bin/minimal-claude
    系统提示词: prompts/calico_cat.md
    状态: idle

  薇薇 (codex)
    管道: pipe_weiwei
    执行命令: ./bin/minimal-codex
    系统提示词: prompts/lihua_cat.md
    状态: idle

  小乔 (gemini)
    管道: pipe_xiaoqiao
    执行命令: ./bin/minimal-gemini
    系统提示词: prompts/silver_cat.md
    状态: idle
```

### 发送任务

```bash
# 发送任务给花花
./bin/cat-cafe --send --to 花花 --task "实现一个 HTTP 服务器"

# 发送任务给薇薇
./bin/cat-cafe --send --to 薇薇 --task "审查 server.go 的安全性"

# 发送任务给小乔
./bin/cat-cafe --send --to 小乔 --task "设计登录页面"
```

### 启动 Agent Worker

在不同的终端窗口中启动各个 Agent:

```bash
# 终端 1: 启动花花
./bin/cat-cafe --mode agent --agent 花花

# 终端 2: 启动薇薇
./bin/cat-cafe --mode agent --agent 薇薇

# 终端 3: 启动小乔
./bin/cat-cafe --mode agent --agent 小乔
```

---

## 🔄 工作流程

### 完整流程示例

1. **启动所有 Agent**

```bash
# 终端 1
./bin/cat-cafe --mode agent --agent 花花

# 终端 2
./bin/cat-cafe --mode agent --agent 薇薇

# 终端 3
./bin/cat-cafe --mode agent --agent 小乔
```

2. **发送任务给花花进行架构设计**

```bash
./bin/cat-cafe --send --to 花花 --task "设计一个用户认证系统"
```

花花会:
- 分析需求
- 设计系统架构
- 实现核心代码
- 输出设计文档

3. **发送任务给薇薇进行代码审查**

```bash
./bin/cat-cafe --send --to 薇薇 --task "审查花花设计的用户认证系统"
```

薇薇会:
- 检查代码质量
- 发现安全漏洞
- 提出改进建议
- 设计测试方案

4. **发送任务给小乔进行 UI 设计**

```bash
./bin/cat-cafe --send --to 小乔 --task "设计用户认证系统的登录界面"
```

小乔会:
- 设计 UI 界面
- 规划交互流程
- 提供视觉方案
- 给出实现建议

---

## 🧪 运行测试

```bash
# 启动 Redis
redis-server --daemonize yes

# 运行所有测试
cd test && go test -v -count=1

# 查看测试报告
cat ../TEST_REPORT.md
```

---

## 🛠 高级用法

### 自定义配置文件

```bash
./cat-cafe --config custom_config.yaml --list
```

### 添加新 Agent

1. 创建系统提示词文件 `prompts/new_agent.md`
2. 在 `config.yaml` 中添加配置:

```yaml
agents:
  - name: "新猫咪"
    pipe: "pipe_new"
    exec_cmd: "./new-agent-cli"
    system_prompt_path: "prompts/new_agent.md"
```

3. 重启调度器即可

### 监控 Redis 队列

```bash
# 查看所有 Stream
redis-cli KEYS "pipe:*"

# 查看特定管道的消息
redis-cli XREAD COUNT 10 STREAMS pipe:pipe_huahua 0

# 查看消费者组信息
redis-cli XINFO GROUPS pipe:pipe_huahua
```

---

## 🐛 故障排查

### Redis 连接失败

```bash
# 检查 Redis 是否运行
redis-cli ping

# 如果没有运行，启动 Redis
redis-server --daemonize yes
```

### Agent 无法启动

```bash
# 检查配置文件
cat config.yaml

# 检查系统提示词文件是否存在
ls -la prompts/

# 检查执行命令是否存在
ls -la minimal-*
```

### 任务发送失败

```bash
# 检查 Agent 名称是否正确
./cat-cafe --list

# 检查 Redis 连接
redis-cli ping

# 查看 Redis 日志
tail -f /usr/local/var/log/redis.log
```

---

## 📊 性能优化

### 调整 Redis 配置

编辑 `/usr/local/etc/redis.conf`:

```conf
# 增加最大内存
maxmemory 2gb

# 设置淘汰策略
maxmemory-policy allkeys-lru

# 启用持久化
save 900 1
save 300 10
save 60 10000
```

### 并发处理

启动多个相同 Agent 的 Worker:

```bash
# 启动 3 个花花 Worker
./cat-cafe --mode agent --agent 花花 &
./cat-cafe --mode agent --agent 花花 &
./cat-cafe --mode agent --agent 花花 &
```

---

## 🔐 安全建议

1. **配置文件权限**
   ```bash
   chmod 600 config.yaml
   ```

2. **Redis 密码保护**
   ```yaml
   redis:
     addr: "localhost:6379"
     password: "your-strong-password"
     db: 0
   ```

3. **系统提示词保护**
   ```bash
   chmod 600 prompts/*.md
   ```

---

## 📚 参考文档

- [SPEC.md](SPEC.md) - 系统设计规范
- [TEST_SPEC.md](TEST_SPEC.md) - 测试规范
- [TEST_REPORT.md](TEST_REPORT.md) - 测试报告
- [README.md](README.md) - 项目说明

---

## 💡 最佳实践

1. **任务描述要清晰**: 提供详细的需求描述
2. **合理分配任务**: 根据 Agent 职责分配任务
3. **监控队列状态**: 定期检查 Redis 队列
4. **日志记录**: 保存 Agent 输出日志
5. **定期清理**: 清理已完成的任务消息

---

## 🎉 示例场景

### 场景 1: 开发新功能

```bash
# 1. 花花设计架构
./cat-cafe --send --to 花花 --task "设计博客系统的文章管理模块"

# 2. 薇薇审查代码
./cat-cafe --send --to 薇薇 --task "审查文章管理模块的代码"

# 3. 小乔设计界面
./cat-cafe --send --to 小乔 --task "设计文章编辑器界面"
```

### 场景 2: Bug 修复

```bash
# 1. 薇薇分析问题
./cat-cafe --send --to 薇薇 --task "分析登录失败的 bug"

# 2. 花花修复代码
./cat-cafe --send --to 花花 --task "修复登录验证逻辑"

# 3. 薇薇验证修复
./cat-cafe --send --to 薇薇 --task "验证登录 bug 是否修复"
```

### 场景 3: 代码重构

```bash
# 1. 花花重构代码
./cat-cafe --send --to 花花 --task "重构用户模块，提高可维护性"

# 2. 薇薇审查重构
./cat-cafe --send --to 薇薇 --task "审查重构后的代码质量"

# 3. 小乔更新文档
./cat-cafe --send --to 小乔 --task "更新用户模块的 UI 文档"
```
