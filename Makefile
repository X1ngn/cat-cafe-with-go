...
build:
	@echo "🔨 编译猫猫咖啡屋..."
	go mod download
	go build -o bin/cat-cafe src/main.go src/scheduler.go src/agent_worker.go src/user_interface.go src/api_server.go src/logger.go src/mode_interface.go src/mode_registry.go src/mode_free_discussion.go src/orchestrator.go src/invoke.go src/cli_adapter.go src/websocket.go src/session_persistence.go src/workspace.go src/session_chain.go src/session_chain_storage.go src/session_chain_context.go src/token_estimator.go
